...
export default {
  ...
  methods: {
    ...
    hideTol() {
      setTimeout(
        () =>
          (document.querySelector(".BMap_noprint.anchorTR").style.display =
            "none"),
        200
      );
    },
    initMap() {
      ...
      this.move(this.cars[0], this.cars[0].trackPoints)();
      setTimeout(function () {
        // layer.msg("鼠标滚轮缩放", { offset: "b" });
      }, 1000);
    },
    move(car, trackPoints = []) {
      if (!trackPoints.length) return (_) => _;
      return function () {
        let t,
          o = 0;
        t = setInterval(() => {
